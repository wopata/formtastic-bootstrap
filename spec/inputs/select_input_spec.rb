...
describe 'select input' do
  ...
  describe 'explicit values' do
    describe 'using an array of values' do
      ...
      it 'should have a option for each key and/or value' do
        @array_with_values.each do |v|
          output_buffer.should have_tag("form div div.input select option[@value='#{v}']", /^#{v}$/)
        end
        @array_with_keys_and_values.each do |v|
          output_buffer.should have_tag("form div div.input select option[@value='#{v.second}']", /^#{v.first}$/)
        end
      end
    end
    ...
    describe "using a related model without reflection's options (Mongoid Document)" do
      ...
      it 'should draw select options' do
        output_buffer.should have_tag('form div div.input select')
        output_buffer.should have_tag('form div div.input select#post_reviewer_id')
        output_buffer.should_not have_tag('form div div.input select#post_mongoid_reviewer_id')
      end
    end
  ...
  describe 'for boolean columns' do
    ...
    #     output_buffer.should have_tag("form div div.input select option[@value='true']", /^Yes$/)
    #     output_buffer.should have_tag("form div div.input select option[@value='false']", /^No$/)
    #   end
    # end
    ...
    describe 'custom locale' do
      ...
      it 'should render a select with at least options: true/false' do
        output_buffer.should have_tag("form div div.input select option[@value='true']", /#{@boolean_select_labels[:yes]}/)
        output_buffer.should have_tag("form div div.input select option[@value='false']", /#{@boolean_select_labels[:no]}/)
      end
    end
  end
  ...
  describe 'for a belongs_to association' do
    ...
    it 'should have a select inside the wrapper' do
      output_buffer.should have_tag('form div div.input select')
      output_buffer.should have_tag('form div div.input select#post_author_id')
      output_buffer.should have_tag('form div div.input select#post_reviewer_id')
    end

    it 'should have a valid name' do
      output_buffer.should have_tag("form div div.input select[@name='post[author_id]']")
      output_buffer.should_not have_tag("form div div.input select[@name='post[author_id][]']")
      output_buffer.should_not have_tag("form div div.input select[@name='post[reviewer_id][]']")
    end

    it 'should not create a multi-select' do
      output_buffer.should_not have_tag('form div div.input select[@multiple]')
    end

    it 'should create a select without size' do
      output_buffer.should_not have_tag('form div div.input select[@size]')
    end

    it 'should have a blank option' do
      output_buffer.should have_tag("form div div.input select option[@value='']")
    end

    it 'should have a select option for each Author' do
      output_buffer.should have_tag("form div div.input select[@name='post[author_id]'] option", :count => ::Author.all.size + 1)
      ::Author.all.each do |author|
        output_buffer.should have_tag("form div div.input select option[@value='#{author.id}']", /#{author.to_label}/)
      end
    end

    it 'should have one option with a "selected" attribute (bob)' do
      output_buffer.should have_tag("form div div.input select[@name='post[author_id]'] option[@selected]", :count => 1)
    end
    ...
    it 'should not singularize the association name' do
      ...
      output_buffer.should have_tag('form div div.input select#post_author_status_id')
    end
  end
  ...
  describe 'for a belongs_to association with :group_by => :continent' do
    ...
    0.upto(1) do |i|
      it 'should have all option groups and the right values' do
        output_buffer.should have_tag("form div div.input select optgroup[@label='#{@continent_names[i]}']", @authors[i].to_label)
      end

      it 'should have custom group labels' do
        output_buffer.should have_tag("form div div.input select optgroup[@label='#{@continents[i].id}']", @authors[i].to_label)
      end

      it 'should have custom author labels' do
        output_buffer.should have_tag("form div div.input select optgroup[@label='#{@continent_names[i]}']", @authors[i].login)
      end

      it 'should have custom author and group labels' do
        output_buffer.should have_tag("form div div.input select optgroup[@label='#{@continents[i].id}']", @authors[i].login)
      end
    end

    it 'should have no duplicate groups' do
      output_buffer.should have_tag('form div div.input select optgroup', :count => 8)
    end

    it 'should sort the groups on the label method' do
      output_buffer.should have_tag("form div div.input select optgroup[@label='Africa']")
      output_buffer.should have_tag("form div div.input select optgroup[@label='99']")
    end
  ...
  describe 'for a has_many association' do
    ...
    it 'should have a select inside the wrapper' do
      output_buffer.should have_tag('form div div.input select')
      output_buffer.should have_tag('form div div.input select#author_post_ids')
    end

    it 'should have a multi-select select' do
      output_buffer.should have_tag('form div div.input select[@multiple="multiple"]')
    end

    it 'should append [] to the name attribute for multiple select' do
      output_buffer.should have_tag('form div div.input select[@multiple="multiple"][@name="author[post_ids][]"]')
    end

    it 'should have a select option for each Post' do
      output_buffer.should have_tag('form div div.input select option', :count => ::Post.all.size)
      ::Post.all.each do |post|
        output_buffer.should have_tag("form div div.input select option[@value='#{post.id}']", /#{post.to_label}/)
      end
    end

    it 'should not have a blank option by default' do
      output_buffer.should_not have_tag("form div div.input select option[@value='']")
    end
    ...
    it 'should have one option with a "selected" attribute' do
      output_buffer.should have_tag('form div div.input select option[@selected]', :count => 1)
    end
  end
  ...
  describe 'for a has_and_belongs_to_many association' do
    ...
    it 'should have a select inside the wrapper' do
      output_buffer.should have_tag('form div div.input select')
      output_buffer.should have_tag('form div div.input select#post_author_ids')
    end
    ...
    it 'should have a select option for each Author' do
      output_buffer.should have_tag('form div div.input select option', :count => ::Author.all.size)
      ::Author.all.each do |author|
        output_buffer.should have_tag("form div div.input select option[@value='#{author.id}']", /#{author.to_label}/)
      end
    end
  ...
  describe 'when :prompt => "choose something" is set' do
    ...
    it 'should have a select with prompt' do
      output_buffer.should have_tag("form div div.input select option[@value='']", /choose author/, :count => 1)
    end
  ...
  describe 'when no object is given' do
    ...
    it 'should generate label' do
      output_buffer.should have_tag('form div label', /Author/)
      output_buffer.should have_tag("form div label[@for='project_author']")
    end

    it 'should generate select inputs' do
      output_buffer.should have_tag('form div div.input select#project_author')
      output_buffer.should have_tag('form div div.input select option', :count => ::Author.all.size + 1)
    end
  ...
  describe 'when no association exists' do
    ...
    it 'should still generate a valid name attribute' do
      ...
      output_buffer.should have_tag("form div div.input select[@name='project[author_name]']")
    end
    ...
    describe 'and :multiple is set to true through :input_html' do
      it "should make the select a multi-select" do
        ...
        output_buffer.should have_tag("form div div.input select[@multiple]")
      end
    end
  ...
  describe 'when a grouped collection collection is given' do
    ...
    it 'should generate an option to each item' do
      @grouped_opts.each do |opt_pair|
        output_buffer.should have_tag("form div div.input select optgroup[@label='#{opt_pair[0]}']")
        opt_pair[1].each do |v|
          output_buffer.should have_tag("form div div.input select optgroup[@label='#{opt_pair[0]}'] option[@value='#{v}']")
        end
      end
      output_buffer.should have_tag("form div div.input select optgroup option[@selected]","hands")
    end
  end
  ...
  describe "enum" do
    ...
    describe ":as is not set" do
      ...
      it "should render a select field" do
        output_buffer.should have_tag("form div div.input select", :count => 2)
      end
    end
    ...
    describe ":as is set" do
      ...
      it "should render a text field" do
        output_buffer.should have_tag("form div div.input input[@type='text']", :count => 2)
      end
    end
  end
